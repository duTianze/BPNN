...
public class NeuronNet {
	 ...
	 * 误差停止阈值
	 */
	public static final double STOP = 0.000001;
	/**
	 * 迭代次数阈值
	 */
	public static final int NUMBER_ROUND = 200000;
		...
		for (int i = 0; i < layer-1; ++ i) {
			...
			for (int j = 0; j < lens[i]; ++ j) {
				...
				for (int k = 0; k < lens[i+1]; ++ k) {
					weight[i][j][k] = RandomUtil.uniform(-0.5, 0.5);
				}
			}
		}
	...
	public void SGD (double [][] features, double [][] labels) {
		...
		while ((num ++) <= NUMBER_ROUND && error > STOP) {
			...
			MainFrame.jTextArea.append("[Info] Times = " + num + ", error = " + error + "\n");
		}
		MainFrame.jTextArea.append("Train finished!");
	}
